				...
				echo '
							<div class="cat_bar">
								<h3 class="', !empty($config_var['class']) ? $config_var['class'] : 'catbg', '"', !empty($config_var['force_div_id']) ? ' id="' . $config_var['force_div_id'] . '"' : '', '>
									', ($config_var['help'] ? '<a href="' . $scripturl . '?action=helpadmin;help=' . $config_var['help'] . '" onclick="return reqOverlayDiv(this.href);" class="help"><i class="fa fa-question-circle fa-lg" title="'.									$txt['help'].'"></i></a>' : ''), '
									', $config_var['label'], '
								</h3>
							</div>';
			}
	...
	echo '
						<div id="section_header" class="cat_bar">
							<h3 class="catbg">
								...
								<i class="fa fa-search fa-lg"></i>&nbsp;', sprintf($txt['admin_search_results_desc'], $context['search_term']), '
							</h3>
						</div>
					<div class="windowbg">';
	...
	foreach ($context['pinfo'] as $area => $php_area)
	{
		...
		foreach ($php_area as $key => $setting)
		{
			...
			if (is_array($setting))
			{
				...
				echo '
								<tr>
									<td align="left" width="33%" class="windowbg">', $key, '</td>';

				foreach ($setting as $key_lm => $value)
				{
					echo '
									<td align="left" width="33%" class="windowbg">', $value, '</td>';
				}
				echo '
								</tr>';
			}
			...
			else
			{
				echo '
								<tr>
									<td align="left" width="33%" class="windowbg">', $key,  '</td>
									<td align="left" class="windowbg">', $setting, '</td>
								</tr>';
			}
		}
		echo '
	...
	if ($context['user']['is_admin'])
		echo '
							<object id="quick_search">
								<form action="', $scripturl, '?action=admin;area=search" method="post" accept-charset="', $context['character_set'], '" class="floatright">
									<i class="fa fa-search fa-lg centericon"></i>
